...
import { Calendar, ChevronLeft, ChevronRight, BookOpen, Globe } from "lucide-react";
...
const CalendarPage: React.FC = () => {
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 1; i <= daysCount; i++) {
      ...
      days.push({ day: i, date, dateString, events: events.filter((e) => e.date === dateString) });
    }
    return days;
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-primary-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <p className="text-white/70 mt-1 text-sm">Events, schedules and important dates</p>
        </div>
      </div>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5  font-semibold bg-primary-700 text-white rounded hover:bg-primary-800 transition-colors"
                  >
                    Today
                  </button>
                  <button
              ...
              {loading ? (
                ...
                <div className="p-4">
                  <div className="grid grid-cols-7 mb-2">
                    {weekdays.map((day, i) => (
                      <div key={day} className={`text-center  font-semibold py-2 ${i >= 5 ? "text-red-500" : "text-gray-500"}`}>
                        {day}
                      </div>
                    ))}
                  </div>
                    ...
                    {calendarDays.map((day, index) => (
                      <div
                        ...
                        className={`min-h-16 rounded p-1  ${
                          day.day === 0 ? "bg-transparent" :
                          day.date && isToday(day.date) ? "bg-primary-700 text-white" :
                          "bg-white hover:bg-gray-50 border border-gray-100"
                        }`}
                      >
                        {day.day > 0 && (
                          <>
                            <div className={`text-right mb-1 font-semibold ${day.date && isToday(day.date) ? "text-white" : "text-gray-700"}`}>
                              {day.day}
                            </div>
                            ...
                            {day.events && day.events.length > 2 && (
                              <div className="text-[9px] text-gray-400">+{day.events.length - 2}</div>
                            )}
                          </>
                        )}
                      </div>
          ...
          <div>
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
                ...
                <span className="ml-auto  bg-primary-50 text-primary-700 font-semibold px-2 py-0.5 rounded">
              ...
              {loading ? (
                <div className="flex justify-center py-8">
                  <div className="animate-spin rounded-full h-6 w-6 border-2 border-primary-700 border-t-transparent" />
                </div>
              ) : upcomingEvents.length === 0 ? (
                <div className="text-center py-10 text-gray-500 text-sm">No upcoming events.</div>
              ) : (
                      ...
                      <div key={event.id} className="flex items-start gap-3 px-5 py-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className=" text-gray-500 flex items-center gap-1 mt-0.5">
                            {event.subjects ? (
                              <><BookOpen className="h-3 w-3 text-gray-400" />{event.subjects.code}</>
                            ) : (
                              <><Globe className="h-3 w-3 text-gray-400" />General</>
                            )}
                          </span>
                        </div>
                      </div>
